...
import { Users, Clock, CheckCircle, CaretLeft, CaretRight, Plus } from '@phosphor-icons/react'
...
export default function DetailHTSKStudentRegistration({ data, unitId, taskId, semesterId, semesterDisplay }) {
    ...
    const run = async () => {
      ...
      try {
        ...
        const rows = submittedMsvSorted.map((id) => {
          ...
          if (!m) return { student_id: id, full_name: '—', class_name: '—', missing: true }
          return { student_id: id, full_name: m.full_name || '—', class_name: m.class_name || '—', missing: false }
        })
        setSubmittedRows(rows)
      } catch (err) {
        if (!cancelled) message.error(err?.message || 'Lỗi tải dữ liệu sinh viên.')
      } finally {
        if (!cancelled) setSubmittedRowsLoading(false)
      }
    }
    run()
    return () => {
  ...
  const handleOpenRespond = (editing = false) => {
    ...
    if (canEditExisting) {
      for (const row of submittedRows) {
        ...
        if (!id) continue
        initialMeta[id] = { full_name: row?.full_name || '', class_name: row?.class_name || '' }
      }
    }
  ...
  const handleGoReview = async () => {
    ...
    if (combined.size === 0) {
      message.warning('Vui lòng chọn hoặc dán ít nhất một MSV.')
      return
    }
    ...
    try {
      ...
      message.error(err?.message || 'Lỗi kiểm tra MSV.')
    } finally {
  ...
  const handleAddMsvInReview = () => {
    ...
    if (invalid.length) {
      message.error(`MSV không hợp lệ: ${invalid.slice(0, 5).join(', ')}`)
      return
    }
    const toAdd = ids.filter((id) => !selectedStudentIds.has(id))
    if (!toAdd.length) {
      message.info('MSV đã có trong danh sách.')
      return
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!list_MSV.length) {
      message.warning('Chọn ít nhất một MSV.')
      return
    }
    if (!content) {
  ...
  const { statusText, statusVariant } = useMemo(() => {
    ...
    if (submission?.status === 'WAITING') return { statusText: 'Đang chờ đăng ký', statusVariant: 'pending' }
    ...
    <div className={u.detailRoot}>
        ...
        <div className={u.cardBody}>
          <div className={u.infoGrid}>
            <div className={u.infoItem}>
              ...
              <span className={`${styles.statusBadge} ${styles[`statusBadge_${statusVariant}`] || ''}`}>{statusText}</span>
            </div>
            <div className={u.infoItem}>
            ...
            <div className={u.infoItem}>
              ...
              <span className={u.infoValue}>{formatDateTime(data?.event_start)} - {formatDateTime(data?.event_end)}</span>
            </div>
            <div className={u.infoItem}>
              <span className={u.infoLabel}>Thời gian đăng ký</span>
              <span className={u.infoValue}>{formatDateTime(data?.registration_start)} - {formatDateTime(data?.registration_end)}</span>
            </div>
            <div className={u.infoItem}>
              <span className={u.infoLabel}>SV chủ động đăng ký</span>
              <span className={u.infoValue}>Có</span>
            </div>
            <div className={u.infoItem}>
              <span className={u.infoLabel}>Giới hạn SV/đơn vị</span>
              <span className={u.infoValue}>{data?.limit_student_registration_in_one_unit ?? '—'}</span>
            </div>
            <div className={u.infoItem}>
          ...
          <div className={u.readonlyBlock} style={{ marginBottom: '1.25rem', padding: '1rem 1.25rem' }}>
            ...
            <button className={u.primaryBtn} onClick={handleCopyStudentRegistrationLink}>Sao chép link đăng ký</button>
          </div>
          {data?.description && (
      ...
      <section className={u.surfaceCard}>
        <div className={u.cardHeaderBar}>
          ...
          {submission && !showRespondForm && !isWaitingSubmission ? <button className={u.primaryBtn} onClick={() => handleOpenRespond(true)}>Chỉnh sửa</button> : null}
        </div>
        <div className={u.cardBody}>
          {submissionLoading ? (
            ...
            <div className={u.submissionView}>
              <div className={`${u.submissionBanner} ${isWaitingSubmission ? u.bannerInfo : u.bannerSuccess}`}>
                ...
                <span>{isWaitingSubmission ? 'Đang trong thời gian đợi sinh viên đăng ký. Danh sách sẽ tự động nộp khi hết thời gian đăng ký sự kiện. Bạn không có quyền sửa danh sách này.' : 'Đơn vị đã gửi phản hồi nhiệm vụ này thành công.'}</span>
              </div>
              <div className={u.readonlyBlock}>
                <div className={u.readonlyLabel}><Users size={18} weight="bold" /> Danh sách đoàn viên tham gia ({submittedMsvSorted.length})</div>
                {submittedRowsLoading ? (
                  <div className={u.loading}><Clock size={24} className="spin-slow" /><p>Đang đối chiếu dữ liệu thành viên...</p></div>
                ) : (
                  <div className={u.memberTableWrap}>
                    <table className={u.memberTable}>
                      <thead><tr><th>MSV</th><th>Họ tên</th><th>Lớp</th></tr></thead>
                      <tbody>
                        {submittedRows.map((row) => (
                          <tr key={row.student_id}>
                            <td><span style={{ fontWeight: 800, color: '#2563eb' }}>{row.student_id}</span></td>
            ...
            <div className={u.formStack}>
              <div className={u.callout}>Chọn đoàn viên từ danh sách hoặc dán danh sách mã sinh viên vào ô dưới đây.</div>
              ...
              <div className={u.pasteBlock}>
                ...
                <div className={u.pasteActions}>
                  <button type="button" className={u.primaryBtn} onClick={handleGoReview} disabled={validatingReview}>
            ...
            <div className={`${u.formStack} ${u.reviewStack}`}>
              ...
              <div className={u.addMsvRow}>
                ...
                <button className={u.secondaryBtn} onClick={handleAddMsvInReview}><Plus size={16} weight="bold" /> Thêm</button>
              </div>
                  ...
                  <tbody>
                    {Array.from(selectedStudentIds).sort(compareStudentId).map((id) => (
                      <tr key={id}>
                        ...
                        <td><button type="button" className={u.removeRowBtn} onClick={() => setSelectedStudentIds((prev) => { const n = new Set(prev); n.delete(id); return n })}>Xóa</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
